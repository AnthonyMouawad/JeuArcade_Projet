...
def activer():
    bouton5.config(state="normal")
    bouton6.config(state="normal")
    bouton7.config(state="normal")
...
def jouer(coup_utilisateur):
    ...
    if score_ordinateur == 3:
        desactiver()
        messagebox.showinfo("Perdu!", "L'ordinateur a gagné!")
...
homer1 = Image.open("img/wallpaper.jpg")
...
# images
rien = ImageTk.PhotoImage(Image.open('img/fond-rien.jpg'))
rien1 = ImageTk.PhotoImage(Image.open('img/fond-rien.jpg'))
versus = ImageTk.PhotoImage(Image.open('img/VS.png'))
pierre = ImageTk.PhotoImage(Image.open('img/pierrev2.png'))
papier = ImageTk.PhotoImage(Image.open('img/papierv2.png'))
ciseaux = ImageTk.PhotoImage(Image.open('img/ciseauxv2.png'))
rougep = ImageTk.PhotoImage(Image.open('img/rougepierre.png'))
rougef = ImageTk.PhotoImage(Image.open('img/rougepapier.png'))
rougec = ImageTk.PhotoImage(Image.open('img/rougeciseaux.png'))
...
libelle_affiche = tk.Label(fenetre, text="Menu Principal", font=(
    "Verdana", 30, 'italic bold'), bg='red', fg='white')
libelle_affiche.place(x=600, y=40, width=340, height=45)


# boutton règles du jeu
bouton = tk.Button(fenetre, text='Règles du jeu', font=(
    "Helvetica", 13), width=80, height=20, command=regle)
bouton.place(x=650, y=200, width=240, height=40)


# boutton lancer le jeu
bouton1 = tk.Button(fenetre, text='Lancer le jeu', font=(
    "Helvetica", 13), width=80, height=20, command=jeu)
...
# boutton recommencer
bouton4 = tk.Button(fenetre2, text='Recommencer',
                    font=("Helvetica", 10), command=reinit)
bouton4.place(x=1380, y=10, width=100, height=40)


# boutton pour quitter le jeu
bouton3 = tk.Button(fenetre, text='Quitter', fg='red', font=(
    "Helvetica", 13), width=80, height=20, command=quitter)
...
texte3 = tk.Label(
    fenetre2, text="Pour jouer, cliquez sur une des icones ci-dessous.", font=("Helvetica", 10))
...
2.    Le jouer qui a accumulé 3 points à gagner 
...
libelle_affiche1 = tk.Label(fenetre1, text=texte, bg='black', fg='white', font=(
    "Helvetica", 16), anchor='w', justify=tk.CENTER, wraplength=695)
